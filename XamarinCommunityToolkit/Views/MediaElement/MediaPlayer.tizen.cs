...
namespace Xamarin.CommunityToolkit.Tizen.UI.Views
{
	...
	{
		public static readonly BindableProperty SourceProperty = BindableProperty.Create(nameof(Source), typeof(CommunityToolkit::MediaSource), typeof(MediaPlayer), default(CommunityToolkit::MediaSource), propertyChanged: OnSourceChanged);
		...
		[Xamarin.Forms.TypeConverter(typeof(CommunityToolkit::MediaSourceConverter))]
		public CommunityToolkit::MediaSource Source
		{
			get => (CommunityToolkit::MediaSource)GetValue(SourceProperty);
		...
		{
			ShowController();
			return impl.Seek(ms).ContinueWith((t) => Position = impl.Position);
		}

		public Task<bool> Start() => impl.Start();

		public void Stop() => impl.Stop();

		public Task<Stream> GetAlbumArts() => impl.GetAlbumArts();

		public Task<IDictionary<string, string>> GetMetadata() => impl.GetMetadata();

		public Task<Size> GetVideoSize() => impl.GetVideoSize();
		...
		void UpdateAutoPlay() => impl.AutoPlay = AutoPlay;

		void UpdateAutoStop() => impl.AutoStop = AutoStop;

		void UpdateIsMuted() => impl.IsMuted = IsMuted;

		void UpdateIsLooping() => impl.IsLooping = IsLooping;
		...
		void OnSourceChanged(object sender, EventArgs e) => impl.SetSource(Source);
		...
		void OnVolumeChanged() => impl.Volume = Volume;

		void OnAspectModeChanged() => impl.AspectMode = AspectMode;
		...
		static void OnSourceChanged(BindableObject bindable, object oldValue, object newValue) => (bindable as MediaPlayer)?.OnSourceChanged(bindable, EventArgs.Empty);
	}
}
