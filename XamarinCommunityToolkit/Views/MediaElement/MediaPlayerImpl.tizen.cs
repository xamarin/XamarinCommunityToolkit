...
using Log = Xamarin.Forms.Platform.Tizen.Log;
...
namespace Xamarin.CommunityToolkit.Tizen.UI.Views
{
	public class MediaPlayerImpl : IPlatformMediaPlayer
	{
		...
		CommunityToolkit::MediaSource source;
		...
		Forms.VisualElement TargetView => VideoOutput?.MediaView;
		...
		public void SetDisplay(IVideoOutput output) => VideoOutput = output;

		public async Task<int> Seek(int ms)
		{
			try
			{
				await player.SetPlayPositionAsync(ms, false);
			}
			catch (Exception e)
			{
		...
		public void SetSource(CommunityToolkit::MediaSource source)
		{
			this.source = source;
		...
		public async Task<Forms.Size> GetVideoSize()
		{
			if (player.State == PlayerState.Idle)
			{
				if (tcsForStreamInfo == null || tcsForStreamInfo.Task.IsCompleted)
			...
			return new Xamarin.Forms.Size(videoSize.Width, videoSize.Height);
		}
		...
		void ApplySource()
		{
			...
			if (source is CommunityToolkit::UriMediaSource uriSource)
			{
				var uri = uriSource.Uri;
				player.SetSource(new MediaUriSource(uri.IsFile ? uri.LocalPath : uri.AbsoluteUri));
			}
			else if (source is CommunityToolkit::FileMediaSource fileSource)
			{
		...
		void OnBufferingProgressChanged(object sender, BufferingProgressChangedEventArgs e) => BufferingProgressUpdated?.Invoke(this, new BufferingProgressUpdatedEventArgs { Progress = e.Percent / 100.0 });

		void OnPlaybackCompleted(object sender, EventArgs e) => PlaybackCompleted?.Invoke(this, EventArgs.Empty);
